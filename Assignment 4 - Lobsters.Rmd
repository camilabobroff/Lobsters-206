...
**Figure 1. Lobster Abundance in the Santa Barbara Channel (2012 - 2017).** Counts of California spiny lobster collected annually in late summer before the fishing season (data collected by SBC LTER)^3^.
```{r echo = FALSE, message = FALSE}
#Trap Totals
...
**Figure 2. Lobster Traps in the Santa Barbara Channel (2012 - 2017).** Counts of commercial trap floats collected every two to four weeks during the fishing season (data collected by SBC LTER)^3^.
...
lob_aov_table <- xtable(lobster_aov, caption = "2017 Mean Lobster Size ANOVA results summary.")
print(lob_aov_table, comment = FALSE, caption.placement = "top")
...
lob_size_graph <- lob_size_summary %>% 
   ...
   geom_col(fill = "lightsteelblue3", colour = "black", aes(fill = SITE), position = "dodge", show.legend = FALSE) +
...
```
Mean lobster lengths in Isla Vista (mean ± sd [units], n = sample size) and Mohawk Reef (mean ± sd [units], n = sample size) differ signifcantly in 2012 and 2017 [t(`r IVEE_ttest$parameter`) = `r round(IVEE_ttest$statistic,2)`, *p* = `r round(IVEE_ttest$p.value,3)`, $\alpha$ = 0.05], t(`r MOHK_ttest$parameter`) = `r round(MOHK_ttest$statistic,2)`, *p* = `r round(MOHK_ttest$p.value,3)`, $\alpha$ = 0.05, respectively]). Despite the significant findings that lobster length in 2017 was larger than 2012 for Isla Vista, an MPA site, there is a small effect size and number of observations was very low for 2012 (6 observations). On the other hand, lobsters at Mohawk Reef (a non-MPA site) were significantly smaller in 2017 than in 2012, with a medium effect size. Given these results, while there is an overall increase in lobster lengths at MPA sites, the magnitude of this increase is small. However, the decrease in lobster size in non-MPA sites in combination with the medium effect size is notable.
(NOTE TO SELF:NEEDS TO BE RE-WORDED)
...
```
There is a significant association between observed lobsters above or below the legal limit (82.6 mm) and location ($\chi^2${`r size_x2$parameter`} = ....., p < 0.001). Notably, ... (ADD MORE HERE)
